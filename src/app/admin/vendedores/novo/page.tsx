...
export default function NovoVendedorPage() {
  ...
  useEffect(() => {
    async function loadStores() {
      try {
        ...
        toast.error('Erro ao carregar a lista de lojas');
      } finally {
  ...
  const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      // Criar o vendedor no banco de dados
      const newSeller = await sellerService.add(formData);
      
      if (newSeller) {
        toast.success('Vendedor criado com sucesso!');
        router.push('/admin/vendedores');
      } else {
        toast.error('Erro ao criar vendedor. Tente novamente.');
      }
    } catch (error) {
  ...
  // Componente de carregamento de lojas
  if (loadingStores) {
    return (
      <AdminPageLayout title="Novo Vendedor">
        <div className="flex justify-center items-center h-64">
          <div className="animate-spin rounded-full h-12 w-12 border-t-2 border-b-2 border-blue-600"></div>
          <span className="ml-3">Carregando lojas...</span>
        </div>
      </AdminPageLayout>
    );
  }
